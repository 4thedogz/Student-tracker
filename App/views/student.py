  ...
  try:
    ...
    if result:
      ...
      return jsonify({"error": "Student creation failed"
                      }), 500  # You can choose an appropriate status code
...
@student_views.route("/api/reviews/<int:student_id>", methods=["POST"])
@jwt_required()
...
@student_views.route('/api/reviews/<int:review_id>/karma', methods=['POST'])
@jwt_required()
